...
START_JUEVES = datetime(2026, 7, 2) # Calendarizamos para que la DAG se ejecute todos los Miercoles
DELTA = timedelta(weeks=2) # La DAG se ejecutará cada 4 semanas a partir de la fecha de inicio

with DAG(
    dag_id="fetch_temp_air",
    description="Fetches Air Temperature from the GEE API using Docker.",
    start_date=START_JUEVES,
    ...
    fetch_temp_air = DockerOperator(
        task_id="fetch_temp_air",
