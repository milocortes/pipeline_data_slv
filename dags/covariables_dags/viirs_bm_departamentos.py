...
START_SABADO = datetime(2026, 7, 4) # Calendarizamos para que la DAG se ejecute todos los Miercoles
DELTA = timedelta(weeks=2) # La DAG se ejecutará cada 4 semanas a partir de la fecha de inicio

with DAG(
    dag_id="fetch_viirs_bm_departamentos",
    description="Fetches Nightmare Lights (Departamentos) from the VIIRS BlackMarble API using Docker.",
    start_date=START_SABADO,
    ...
    fetch_viirs_bm_departamentos = DockerOperator(
        task_id="fetch_viirs_bm_departamentos",
