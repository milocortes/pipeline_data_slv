...
def time_series_plot(data : Dict[str, pl.DataFrame], 
    ...
    data2plot = data.with_columns(pl.col("datetime").dt.year().cast(pl.Utf8) + "Q" + pl.col("datetime").dt.quarter().cast(pl.Utf8) ).to_numpy()
...
def render_evi_gee():
    time_series_plot(geospatial_data["evi_gee"], "evi_gee")

def render_gdp_us_const_trim():
    time_series_plot(geospatial_data["gdp_us_const_trim"], "gdp_us_const_trim")

def render_ndbi_gee():
    time_series_plot(geospatial_data["ndbi_gee"], "ndbi_gee")

def render_ndvi_gee():
    time_series_plot(geospatial_data["ndvi_gee"], "ndvi_gee")

def render_precip():
    time_series_plot(geospatial_data["precip"], "precip")
