...
st.title("Respaldo de Pronósticos")

# 1. Define your Markdown template with Jinja2 placeholders
template_texto = """

Con la finalidad de mantener la integridad de los datos, la aplicación mantiene un respaldo en RustFS del último pronóstico de los Modelos Lineales, 
de Aprendizaje de Máquina y del Modelo Subnacional.

En caso de corrupción de valores en el Google Sheets que alimenta la visualización de Looker Studio, el usuario puede recuperar el estado de las tablas de pronóstico al pulsar el siguente botón:

"""

# 3. Display in Streamlit
st.markdown(template_texto)

if st.button("Recupera Pronósticos"):
    ## Esta información debe ir en un archivo de configuración. Por el momento quedará hardcodeado
    url = f"{AIRFLOW_APISERVER_DOMAIN}:{AIRFLOW_APISERVER_PORT}/api/v2/dags/respaldo_pronostico_gs/dagRuns"
